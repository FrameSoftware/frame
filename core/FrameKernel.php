...
class FrameKernel {
    ...
    public function launch_kernel(){
        try {
            //session_start();
            //if(!isset($_SESSION['auth'])){//si la variabl n'existe pas

             //   header('Location:http://'.$_SERVER['SERVER_NAME'].'/ubmessenger/');
            //}
            ...
            $bundle_name = $response->getBundleName();
            $controller_name = $response->getControllerName();
            $method_name = $response->getMethodeName();
            ...
            $reflect_controlleur2 = new \ReflectionMethod($response->getControllerClass(),'setNumber');
            ...
            if($reflect_controlleur->getParameters()){//si la methode prend des parametres on lance avec GET
                ...
                //$reflect_controlleur2->invoke($controlleur,$i );
